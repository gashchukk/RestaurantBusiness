...
def create_restaurant(*, db: Session = Depends(get_db), restaurant_in: RestaurantCreate,
                      current_user: User = Depends(check_restaurant_owner_role)) -> Any:
    """ Create new restaurant """
    restaurant = restaurant_crud.create_with_owner(db=db, obj_in=restaurant_in,
                                                    owner_id=current_user.id)
    return restaurant


@router.get("/", response_model=List[Restaurant])
def read_restaurants(db: Session = Depends(get_db), skip: int = 0, limit: int = 100) -> Any:
...
def read_owned_restaurants(db: Session = Depends(get_db), current_user: User = Depends(check_restaurant_owner_role)) -> Any:
...
def read_restaurant(*, db: Session = Depends(get_db), id: int) -> Any:
...
def update_restaurant(*, db: Session = Depends(get_db), id: int, restaurant_in: RestaurantUpdate,
                      current_user: User = Depends(check_restaurant_owner_role),) -> Any:
    """ Update a restaurant """
    restaurant = restaurant_crud.get(db=db, id=id)
    if not restaurant:
